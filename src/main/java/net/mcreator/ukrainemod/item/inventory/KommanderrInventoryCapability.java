...
package net.mcreator.ukrainemod.item.inventory;
...
public class KommanderrInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == UkrainemodModItems.KOMMANDERR) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(0) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != UkrainemodModItems.KOMMANDERR;
			}
